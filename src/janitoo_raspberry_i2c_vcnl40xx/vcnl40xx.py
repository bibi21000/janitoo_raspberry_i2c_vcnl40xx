...
class VCLN4000Component(JNTComponent):
    ...
    def start(self, mqttc):
        ...
        try:
            self.vcnl = Adafruit_VCNL40xx.VCNL4000(address=self.values["addr"].data, i2c=self._bus._ada_i2c, busnum=self._bus.get_busnum())
        except Exception:
...
class VCLN4010Component(VCLN4000Component):
    ...
    def start(self, mqttc):
        ...
        self._bus.i2c_acquire()
        try:
            self.vcnl = Adafruit_VCNL40xx.VCNL4010(address=self.values["addr"].data, i2c=self._bus.get_adafruit_i2c(), busnum=self._bus.get_busnum())
        except Exception:
            logger.exception("[%s] - Can't start component", self.__class__.__name__)
        finally:
            self._bus.i2c_release()
